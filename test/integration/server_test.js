...
describe("integration tests", function () {
  ...
  const HORIZON = 'https://horizon-testnet.fonero.org';
  FoneroSdk.Network.useTestNetwork();
  let server = new FoneroSdk.Server(HORIZON);
  let master = FoneroSdk.Keypair.random();
    ...
    server.operations().forAccount(master.publicKey()).limit(1).order('asc')
      ...
      .then(response => {
        ...
        return server.loadAccount(master.publicKey())
          .then(source => {
            let tx = new FoneroSdk.TransactionBuilder(source)
              .addOperation(FoneroSdk.Operation.accountMerge({
  ...
  describe("/transaction", function () {
    it("submits a new transaction", function (done) {
      createNewAccount(FoneroSdk.Keypair.random().publicKey())
        .then(result => {
    ...
    it("submits a new transaction with error", function (done) {
      server.loadAccount(master.publicKey())
        .then(source => {
          ...
          let tx = new FoneroSdk.TransactionBuilder(source)
            .addOperation(FoneroSdk.Operation.createAccount({
              destination: FoneroSdk.Keypair.random().publicKey(),
              startingBalance: "1"
            }))
            .build();
  ...
  describe("/accounts", function () {
    ...
    it("stream accounts", function (done) {
      ...
      let randomAccount = FoneroSdk.Keypair.random();
